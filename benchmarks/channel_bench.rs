...
fn print_results(results: &[BenchmarkResult]) {
    ...
    println!("{:30} | {:20} | {:15} | {:15} | {:15}", "Benchmark", "Throughput (msg/s)", "Avg Latency (ns)", "p99 Latency (ns)", "Exec Time (ms)");
    println!("{:-^100}", "");
    for result in results {
        println!("{:30} | {:20.2} | {:15.2} | {:15.2} | {:15}", result.name, result.throughput, result.avg_latency_ns, result.p99_latency_ns, result.execution_time_ms);
    }
    println!("{:=^100}", "");
}
...
fn benchmark_oxidator_single(config: BenchmarkConfig) -> BenchmarkResult {
    ...
    BenchmarkResult {
        name: format!("Oxidator (Single Producer, {} Consumer{})", config.num_consumers, if config.num_consumers > 1 { "s" } else { "" }),
        throughput,
        avg_latency_ns: avg_latency,
        p99_latency_ns: p99_latency,
        execution_time_ms,
...
fn benchmark_oxidator_multi(config: BenchmarkConfig) -> BenchmarkResult {
    ...
    let p99_latency = percentile(&mut all_latencies, 99.0);
    BenchmarkResult {
        name: format!("Oxidator (Multi Producer: {}, Consumer: {})", config.num_producers, config.num_consumers),
        throughput,
        avg_latency_ns: avg_latency,
        p99_latency_ns: p99_latency,
        execution_time_ms,
...
fn main() {
    ...
    println!("Comparing Oxidator vs Standard Rust Channels");
    ...
    println!("\n\n");
    println!("{:=^100}", "");
    println!("{:=^100}", " FINAL BENCHMARK RESULTS (Oxidator vs Standard) ");
    println!("{:=^100}", "");
    ...
    println!("- Single producer speedup (Oxidator vs Std Channel): {:.2}x", oxidator_vs_std_single);
    let oxidator_vs_std_multi = oxidator_multi_throughput / std_multi_throughput;
    println!("- Multi producer speedup (Oxidator vs Std Channel): {:.2}x", oxidator_vs_std_multi);
    println!("{:=^80}", "");
}
